<script>
	import H2Hoverable from '$lib/components/H2Hoverable.svelte';
</script>

<div class="container mx-auto typography">
	<h1>Mitarbeitende des Parzival-Projekts</h1>

	<H2Hoverable name="Projektleitung"></H2Hoverable>

	<ul>
		<li>
			<a href="mailto:michael.stolz@unibe.ch"> Prof. Dr. Michael Stolz </a> (Universität Bern)
		</li>
	</ul>

	<H2Hoverable name="Wissenschaftliche Ko-Leitung und Ansprechpartner"></H2Hoverable>

	<ul>
		<li>
			<a href="mailto:stefan.abel@unibe.ch"> PD Dr. Stefan Abel </a> (Universität Bern)
		</li>
	</ul>

	<H2Hoverable name="Ko-Leitung im D-A-CH-Projekt (2012–2017)"></H2Hoverable>

	<ul>
		<li>Prof. Dr. Jens Haustein (Friedrich-Schiller-Universität Jena)</li>
		<li>PD Dr. Sonja Glauch (Friedrich-Alexander-Universität Erlangen-Nürnberg)</li>
		<li>Prof. Dr. Martin Schubert (Berlin-Brandenburgische Akademie der Wissenschaften)</li>
	</ul>

	<H2Hoverable name="Programmierung und Webdesign"></H2Hoverable>
	<ul>
		<li>Simone Hiltscher (Universitäten Basel und Bern, 2005 bis 2021)</li>

		<li>Nicolas Kessler (Universität Bern, unterstützend 2018/19)</li>

		<li>Dominique Steinbach (Universität Bern, 2021 bis 2026)</li>

		<li>Philipp Lammerskitten (Universität Bern, 2021 bis 2026)</li>

		<li>
			<a
				href="https://www.dh.unibe.ch/ueber_uns/personen/prof_dr_hodel_tobias/index_ger.html"
				target="_blank">Prof. Dr. Tobias Hodel</a
			> (Digital Humanities, Universität Bern, 2024–2026)
		</li>

		<li>
			<a
				href="https://www.dh.unibe.ch/ueber_uns/personen/daengeli_peter/index_ger.html"
				target="_blank">Peter Dängeli</a
			> (Digital Humanities, Universität Bern, 2024–2026)
		</li>
		<li>
			<a
				href="https://www.dh.unibe.ch/ueber_uns/personen/flick_sebastian/index_ger.html"
				target="_blank">Sebastian Flick</a
			> (Digital Humanities, Universität Bern, 2024–2026)
		</li>
	</ul>

	<H2Hoverable name="Ehemalige wissenschaftliche Mitarbeiterinnen und Mitarbeiter"></H2Hoverable>

	<ul>
		<li>Letizia Carigiet M.A. (Universität Bern)</li>
		<li>Dr. Yen-Chun Chen (Universität Bern)</li>
		<li>
			Prof. Dr. Kathrin Chlench (Universität Bern, jetzt Rheinische Friedrich-Wilhelms-Universität
			Bonn)
		</li>
		<li>Dr. Ann-Kathrin Deininger (Universität Bern)</li>
		<li>Jasmin Eggel M.A. (Universität Bern)</li>
		<li>Dr. Agata Mazurek (Universität Bern)</li>
		<li>Dr. Simon Falch (Universität Bern)</li>
		<li>Sonja Falk M.A. (Friedrich-Alexander-Universität Erlangen-Nürnberg)</li>
		<li>Dr. Richard Fasching (Universität Bern)</li>
		<li>Marius Gehrig M.A. (Universität Bern)</li>
		<li>Dr. Mirjam Geissbühler</li>
		<li>Christiane Klein M.A. (Universität Bern)</li>
		<li>Dr. Jörg Klenk (Berlin-Brandenburgische Akademie der Wissenschaften)</li>
		<li>Simone Lahn M.A. (Friedrich-Alexander-Universität Erlangen-Nürnberg)</li>
		<li>Dr. Judith Lange (Universität Bern)</li>
		<li>Dr. Claudia Lingscheid (Universität Bern)</li>
		<li>Dr. Agata Mazurek (Universität Bern)</li>
		<li>Dr. Christoph Pretzer (Universität Bern)</li>
		<li>Anett Rehner M.A. (Berlin-Brandenburgische Akademie der Wissenschaften)</li>
		<li>Livia Sandri M.A. (Berlin-Brandenburgische Akademie der Wissenschaften)</li>
		<li>PD Dr. Robert Schöller (Universität Bern)</li>
		<li>
			Prof. Dr. Martin Schubert (Berlin-Brandenburgische Akademie der Wissenschaften, jetzt
			Universität Duisburg-Essen)
		</li>
		<li>Christian Sonder M.A. (Universität Bern)</li>
		<li>Dr. Miriam Strieder (Universität Bern)</li>
		<li>Prof. Dr. Gabriel Viehhauser (Universität Bern, jetzt Universität Wien)</li>
		<li>Dr. Lydia Wegener (Universität Bern)</li>
	</ul>
</div>
